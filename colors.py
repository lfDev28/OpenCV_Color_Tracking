...
while True:
   ...
   # frame= cv.resize(frame, (0,0), fx=0.5, fy=0.5)
    ...
    opening = cv.morphologyEx(
        frame_threshold, cv.MORPH_OPEN, kernel)
    closed = cv.morphologyEx(opening, cv.MORPH_CLOSE,
                             closeKernel)

    contours, hierarchy = cv.findContours(
        closed, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)

    if (len(contours) > 0):
        cnt = contours[0]
        x, y, w, h = cv.boundingRect(cnt)
        cv.rectangle(frame, (x, y), (x+w, y+h), (255, 0, 0), 1)
        cv.putText(frame, "X: " + str(x) + " Y: " + str(y), (x, y),
